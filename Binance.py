...
url = url_binance + '?symbol=' + symbol + '&period=' + period + "&LIM" + lim + "&startTime" +  str(startTime) + "&endTime" +  str(endTime)
...
# Armazenando json em Array
df = pd.DataFrame(data)

# Deletando colunas
df.drop(['shortAccount', 'longAccount'], inplace=True, axis=1)

# Ordenando colunas
df = df.reindex(['longShortRatio', 'timestamp', 'symbol'], axis=1)
